...
- Управление AmneziaVPN на удалённом сервере, с помощью подключения по SSH
- Возможность управлять несколькими серверами, переключаясь между ними. Используется соединение с помощью SSH
